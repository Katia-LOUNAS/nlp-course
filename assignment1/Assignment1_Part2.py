...
def question_one():
    ...
    num_unique_tokens = len(set(nltk.word_tokenize(moby_raw)))
...
def question_four():
    ...
    tokens = nltk.word_tokenize(moby_raw)
    freq_dist = nltk.FreqDist(tokens)
    filtered_tokens = [token for token in freq_dist if len(token) > 5 and freq_dist[token] > 150]
...
def question_seven():
    ...
    return total_tokens / len(sentences) if sentences else 0
